...
// Helper: seed the store and render the rail.
function renderRailWith(endpoints: { id: string; url: string; label: string; enabled: boolean; color: string }[]) {
...
describe('EndpointRail — G6 URL subtitle dedup truth table', () => {
  ...
  // Case C: blank label — URL line shown so row is not invisible.
  it('should render URL subtitle when label is blank (defensive fallback)', () => {
    const { container } = renderRailWith([
      { id: 'ep1', url: 'https://foo.com', label: '', enabled: true, color: '#fff' },
    ]);
    const urlSpan = container.querySelector('.rail-row-url');
    expect(urlSpan).not.toBeNull();
    expect(urlSpan?.textContent).toBe('https://foo.com');
  });
